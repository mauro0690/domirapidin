...
window.RapidinSpreadsheet = (function() {
    ...
    function attachCellEvents() {
        ...
        tbody.querySelectorAll('.btn-del-row').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                ...
                const barrioName = currentData[idx] ? currentData[idx].barrio : 'este barrio';

                let confirmed = true;
                if (window.RapidinApp && window.RapidinApp.showConfirmModal) {
                    confirmed = await window.RapidinApp.showConfirmModal({
                        title: 'Eliminar Barrio de Tarifas',
                        message: `¿Estás seguro de eliminar el barrio '${barrioName}' de la tabla de tarifas?`,
                        icon: 'fa-trash-can',
                        isDanger: true,
                        confirmText: 'Eliminar Barrio'
                    });
                }
                if (!confirmed) return;
                ...
                renderSpreadsheet(currentData);
                updateStats(currentData.length);
                if (window.RapidinApp) window.RapidinApp.showToast(`🗑️ Barrio '${barrioName}' removido de la tabla.`);
            });
        });
    }
    ...
    function addRow() {
        ...
        renderSpreadsheet(currentData);
        updateStats(currentData.length);
        if (window.RapidinApp) window.RapidinApp.showToast("✨ Nueva fila agregada a la tabla de tarifas.");
    }
    ...
    async function saveSpreadsheet() {
        ...
        if (window.RapidinApp && window.RapidinApp.showConfirmModal) {
            confirmed = await window.RapidinApp.showConfirmModal({
                title: 'Guardar y Publicar Tarifas',
                message: `¿Deseas guardar y sincronizar la tabla de tarifas para '${selectedClient || 'el negocio'}' en Supabase PostgreSQL?`,
        ...
        const saveBtn = document.getElementById('btn-save-spreadsheet');
        if (saveBtn) {
            saveBtn.disabled = true;
            saveBtn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Guardando y Sincronizando con Supabase...';
        }
        ...
        try {
            ...
            const res = await response.json();
            if (res.status === 'success') {
                window.RapidinApp.showToast("✅ Base de datos de tarifas guardada y sincronizada exitosamente en Supabase.");
            ...
            if (saveBtn) {
                ...
                saveBtn.innerHTML = '<i class="fa-solid fa-floppy-disk"></i> Guardar Cambios en Servidor';
            }
        }
    }
