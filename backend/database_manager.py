...
def load_clientes_db():
    ...
    if conn:
        try:
            rows = conn.run("SELECT id, nombre, slug, codigo_acceso, nit, tipo, direccion_origen, latitud_origen, longitud_origen, archivo_tarifario, total_barrios, direccion, latitud, longitud, foto_perfil, descripcion, categoria FROM clientes ORDER BY id ASC;")
            conn.close()
            clientes = []
            for r in rows:
                clientes.append({
                    ...
                    "nit": r[4],
                    "tipo": r[5],
                    "direccion_origen": r[6],
                    "latitud_origen": r[7],
                    "longitud_origen": r[8],
                    "archivo_tarifario": r[9],
                    "total_barrios": r[10] or 0,
                    "direccion": r[11] or r[6],
                    "latitud": r[12] or r[7],
                    "longitud": r[13] or r[8],
                    "foto_perfil": r[14],
                    "descripcion": r[15],
                    "categoria": r[16]
                })
    ...
    if conn:
        try:
            for c in clientes_list:
                ...
                VALUES (:id, :nombre, :slug, :codigo_acceso, :nit, :tipo, :direccion_origen, :latitud_origen, :longitud_origen, :archivo_tarifario, :total_barrios, :direccion, :latitud, :longitud, :foto_perfil, :descripcion, :categoria)
                ...
                """, id=c.get("id"), nombre=c.get("nombre"), slug=c.get("slug", c.get("nombre","").lower().replace(' ','_')),
